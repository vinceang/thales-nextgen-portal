...
### 2026-07-02 Playable games — GameLauncher + first original (2048) — `games/`, `components/GameLauncher.*`
**Rule/token changed:** New app capability — real, playable in-browser games; plus an **explicit design-rule exemption for the games zone** (see below). No DS component changed.
**Was:** Play was a browse-only gallery — the detail modal's Play button was a stub, like the other galleries.
**Now:** Games can be **launched and actually played**, fully offline (no network, no external assets — ideal for in-flight):
- `src/games/` — self-contained game modules + a `registry.ts` (gameKey → component). First game: **2048** (`Game2048.tsx`) — an original implementation (the mechanic + name are generic; no third-party code/assets). Keyboard (arrows/WASD) + touch swipe, best score persisted to `localStorage` (`thales.game.2048.best`), turn-based (no game-loop timing bugs).
- `components/GameLauncher.tsx` — a full-screen "cabinet" (title, Restart via key-remount, Close + Esc, locks page scroll) that mounts a game by key. Neutral frame so future bundled games share the shell.
- `content/play.ts` — `Game` gains `gameKey?`; catalogue entries with one are real/playable (2048 added, featured in the hero), the rest stay placeholders. Play page shows a **"Playable" badge** on real games and the modal's primary button reads **"Play"** (playable) vs **"Coming Soon"** (placeholder); `onPrimaryAction` added to the shared MediaDetailModal launches the game. New `play.*` i18n (restart/exit/score/best/newGame/keepGoing/gameOver/youWin/hint2048/comingSoon/playable) in en/fr/es.
- **RULE DEVIATION (designer-approved):** games are their own visual zone and are **exempt from the one-accent / status-by-icon constraint** — a game may use whatever colors its mechanics need (e.g. a future Wordle/Simon). 2048 itself voluntarily stays on-brand (monochrome tiles ramping to the one accent). The launcher chrome remains fully token/on-brand. PROPOSED RULE CHANGE upstream: add a "Games are an exempt visual zone (neutral cabinet; game content may use its own palette)" clause to CLAUDE.md.
**Why:** Designer asked for HTML games / a real Play section, OK'd ignoring the one-hue caveat for games, and chose a hybrid (a few real originals + placeholder catalogue). Games are the one section made real by *building* (no data API exists), and self-contained HTML5 games fit an offline in-flight portal perfectly. Verified in-browser: launched 2048, merged tiles, score/best updated and persisted; the "Coming Soon" path confirmed on a placeholder. Next: more originals (Snake, a bespoke destination-trivia) + vetted MIT/keyless bundled games (js13k / Hextris) in `games/<name>/` with license/attribution preserved.
