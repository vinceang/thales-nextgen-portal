...
- Sits in a translucent dark **circle** (36px at the default 16px heart) so it reads over bright posters — circular is the sanctioned radius exception; flat, no shadow. Stops click propagation, so it can overlay a clickable tile without triggering the tile.
